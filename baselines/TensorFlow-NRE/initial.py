...
if dataset == 'NYT':
    embedding_file = './origin_data/NYT/nyt_100d.txt'
elif dataset == 'KBP':
    embedding_file = './origin_data/KBP/wiki_100d.txt'
elif dataset == 'TACRED':
    embedding_file = './origin_data/TACRED/glove.6B.100d.txt'
...
def init():
    ...
    relation2id = {}
    f = open('./origin_data/' + dataset + '/relation2id.txt', 'r', encoding='utf-8')
    while True:
        content = f.readline()
        if content == '':
            break
    ...
    f = open('./origin_data/' + dataset + '/train.txt', 'r', encoding='utf-8')
    ...
    f = open('./origin_data/' + dataset + '/test.txt', 'r', encoding='utf-8')
    count = 0
    while True:
        content = f.readline()
        if content == '':
    ...
    f = open('./data/' + dataset + '/test_q&a.txt', 'w', encoding='utf-8')
    temp = 0
    for i in test_sen:
    ...
    np.save('./data/' + dataset + '/vec.npy', vec)
    np.save('./data/' + dataset + '/train_x.npy', train_x)
    np.save('./data/' + dataset + '/train_y.npy', train_y)
    np.save('./data/' + dataset + '/testall_x.npy', test_x)
    np.save('./data/' + dataset + '/testall_y.npy', test_y)
...
def seperate():
    ...
    np.save('./data/' + dataset + '/train_word.npy', train_word)
    np.save('./data/' + dataset + '/train_pos1.npy', train_pos1)
    np.save('./data/' + dataset + '/train_pos2.npy', train_pos2)

    print('seperating test all data')
    x_test = np.load('./data/' + dataset + '/testall_x.npy')
    test_word = []
    test_pos1 = []
    test_pos2 = []
    ...
    np.save('./data/' + dataset + '/testall_word.npy', test_word)
    np.save('./data/' + dataset + '/testall_pos1.npy', test_pos1)
    np.save('./data/' + dataset + '/testall_pos2.npy', test_pos2)
...
def getans():
    ...
    np.save('./data/' + dataset + '/allans.npy', allans)
