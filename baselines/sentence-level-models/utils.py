...
class Dataset(object):
	def __init__(self, dataname, args, word2id, device, shuffle=False, batch_size=None):
		if batch_size is None:
			batch_size = args['batch_size']
		lower = args['lower']
		self.device = device
		filename = './data/json/%s.json' % dataname
		...
		for batch in batched_data:
			...
			words = self.get_padded_tensor(batch[0], batch_size)
			pos = self.get_padded_tensor(batch[1], batch_size)
			ner = self.get_padded_tensor(batch[2], batch_size)
			subj_pos = self.get_padded_tensor(batch[3], batch_size)
			obj_pos = self.get_padded_tensor(batch[4], batch_size)
