...
import utils
from utils import pos2id, ner2id, rel2id
import sys
...
class BGRU(nn.Module):
	def __init__(self, args, word_emb=None):
		...
		self.gru = nn.GRU(input_size=input_size, hidden_size=hidden, num_layers=num_layers, batch_first=True, dropout=dropout, bidirectional=True)
		...
		self.flinear = nn.Linear(hidden*2, len(rel2id))
